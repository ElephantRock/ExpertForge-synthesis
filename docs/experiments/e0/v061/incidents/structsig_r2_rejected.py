...
def _preserves_coloring(sigma: dict, colors: dict) -> bool:
    """True iff sigma stabilizes the given coloring: for every node x,
    colors[sigma(x)] == colors[x]. Applied at a search node's refined
    coloring (where individualized vertices carry unique colors), this is
    exactly the stabilizer-of-the-individualization-path condition."""
    for x in colors:
        mapped = sigma.get(x, x)
        if colors[mapped] != colors[x]:
            return False
    return True


class _AutomorphismStore:
    """Stores VERIFIED automorphism permutations (node->node dicts) and
    answers stabilizer-aware skip queries. A skip of candidate v (because v'
    was already explored) is justified only by a stored sigma with
    sigma(v) = v' AND sigma preserving the caller's current coloring."""

    def __init__(self):
        self.automorphisms: list[dict] = []

    def add(self, sigma: dict) -> None:
        self.automorphisms.append(sigma)

    def skip_permitted(self, v, explored: list, colors: dict) -> bool:
        for ex in explored:
            for sigma in self.automorphisms:
                if sigma.get(v, v) == ex and _preserves_coloring(sigma, colors):
                    return True
        return False


def _search(colors, facts, rules, query, depth, budget, autos: _AutomorphismStore, incid=None):
    """Exact IR canonical labeling with stabilizer-aware pruning. At each
    non-discrete cell, EVERY member is a branch candidate; a candidate may
    be skipped only when a VERIFIED automorphism maps it to an already-
    explored candidate AND stabilizes the current refined coloring (i.e.,
    lies in the stabilizer of the individualization path). Returns the set
    of (serialization, node-order) leaves; the canonical form is the minimum
    serialization."""
    ...
    for node in sorted(target, key=lambda n: (n[0], n[1])):
        if autos.skip_permitted(node, explored, colors):
            continue
        budget[0] -= 1
        if budget[0] < 0:
        ...
        branched[node] = _h("INDIV", colors[node], node)
        leaves = _search(branched, facts, rules, query, depth, budget, autos, incid)
        best_leaves |= leaves

        # automorphism harvesting: when two branches produce identical leaf
        # serializations, the induced node bijection is verified against the
        # full incidence structure before being stored; stored automorphisms
        # are later filtered by the stabilizer condition at each use site
        anchor = min(best_leaves, key=lambda t: t[0]) if best_leaves else None
...
def variant_canonical(example: dict) -> str:
    ...
    autos = _AutomorphismStore()
    leaves = _search(colors, facts, rules, query, depth, budget, autos, incid)
